...
try:
    ...
    sql_delete_query = """DELETE FROM ext_temp"""
...
cnt=0
for i in range (7, 46):
    text = soup.find_all('tr')[i].find_all('td')[1].text
    value = text.split('°')
    ...
    try:
        ...
        postgres_insert_query = """ INSERT INTO ext_temp (ind, hr, val) VALUES (%s,%s,%s)"""
        ...
        count = cursor.rowcount
        cnt-=1
        print (count, "Record inserted successfully into ext_temp table")

    except (Exception, psycopg2.Error) as error :
        if(connection):
            print("Failed to insert record into ext_temp table", error)
